...
public class IndexController {
	@Autowired
	private ScriptSessList scriptSessList;
	@Autowired
	...
	@Value("${packetReplyTimeout}")
	private int packetReplyTimeout; // millis
	@Value("${chatImageFolder}")
	private String chatImageFolder;
	@Value("${onlineStatus}")
	 ...
	 * public String getUserDashboard(ModelMap map) { return "userDashboard"; }
	 */
	...
	public ModelAndView getcalendar(ModelMap map, HttpServletRequest request) {
		...
		GetWebmailImapquotaResponse wfresponse = folderClient
		...
		// xmppChatClass.performLogin(loginUser.getUserid(),
		// loginUser.getPassword(), onlineStatus);
	...
	public ModelAndView getinbox(ModelMap map, HttpServletRequest request) {
		...
		// System.out.println("********************Quotalimi="+ per
		// System.out.println("********************Quota per="+qper);
		GetWebmailAllMailCountResponse wfre = folderClient
				.getWebmailAllMailCountRequest(host, port, id, pass, fdrnm);
		map.addAttribute("MailCount", "" + (wfre.getAllemailcount()));
		map.addAttribute("QuotaPer", "" + qper);
		map.addAttribute("QuotaLimit", "" + ql);
		map.addAttribute("QuotaUses", "" + qu);
